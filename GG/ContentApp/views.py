...
from ContentApp.models import SmallText, Blog, Expert, Books
...
def introduce(request):
    ...
    if userid:
        p = UserInfo.objects.fifter(Account=userid)
    return render(request, 'content/introduce.html', locals())
...
def full_search(request):
    ...
    if posts:
        for i in posts:
            print(i.title)
    data = {'posts': posts, 'list_header': '关键字 \'{}\' 搜索结果'.format(keywords)}
    return HttpResponse(data)


@csrf_exempt
def introduce_detail(request, pk, detail):
    """
    学习详情页
    :param request: 
    ...
    if request.method == 'POST':
        ...
        if page and tag:
            obj = Books.objects.filter(page=page, tag=tag)
            book_list = []
            if obj:
                for i in obj:
                    # print(i.name, '---------', i.content)
                    logging.info('get_book:', i.name, i.content)
                    book_list.append({'book_name': i.name, 'book_img': i.img, 'book_content': i.content, 'book_author':
                        i.author, 'book_tag': i.tag, 'book_time': str(i.create_time), 'book_touxiang': i.touxiang})
            # print(len(book_list))
        ...
        if int(pk) == 1:
            books = Books.objects.filter(tag=1, page=int(detail))
        if int(pk) == 2:
            books = Books.objects.filter(tag=2, page=int(detail))
        if int(pk) == 3:
            books = Books.objects.filter(tag=3, page=int(detail))
        ps = Books.objects.filter(tag=int(pk))
        # 同时筛选出当前的目录，并且代码高亮
        detail = int(detail)
        now_pk = detail
        pre = detail - 1
        back = detail + 1
        return render(request, 'content/books.html', locals())